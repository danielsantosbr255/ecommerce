...
export default function Home() {
  return (
    <main className="flex flex-1 flex-col my-5 mx-auto w-full px-2 lg:px-4 lg:max-w-10/12 h-full gap-6 items-center">
      ...
      <ProductSession callback={productService.getAll} label="Destaques" icon={<FaStarHalfAlt size={25} />} />

      <Suspense fallback={<BrandsCarousel brands={null} />}>
        <Brands />
      </Suspense>

      <ProductSession callback={productService.getAll} label="Novidades" icon={<FaBullhorn size={25} />} />

      <Suspense fallback={<CategoriesCarousel categories={null} />}>
        <Categories />
      </Suspense>

      <ProductSession callback={productService.getAll} label="Mais Vendidos" icon={<FaChartLine size={25} />} />
    </main>
  );
}
