...
const Input = ({ id, label, type = "text", icon, className, ...props }: InputProps) => {
  ...
  return (
    ...
    >
      ...
      <label
        htmlFor={id}
        className="bg-bg-secondary absolute text-sm text-tx-secondary font-semibold top-0 -translate-y-1/2 z-5 px-1 peer-focus:text-primary transition-all duration-300"
      >
        {label}
      </label>
