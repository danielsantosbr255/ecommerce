...
const NavItem = ({ href, label, icon, onClick }: NavItemProps): JSX.Element => (
  <Link
    href={href}
    onClick={onClick}
    className="flex items-center gap-2 py-2 px-4 text-gray-700 hover:bg-gray-100 hover:text-highlight-n rounded-md transition duration-300"
  >
    {icon} {label}
  </Link>
);
...
const MobileMenu = ({ onClose }: { onClose: () => void }): JSX.Element => (
  <div className="fixed top-24 left-0 right-0 bg-navbar shadow-md rounded-b-md overflow-hidden z-50 transition-all duration-300">
