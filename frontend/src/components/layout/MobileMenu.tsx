...
const NavItem = ({ href, label, icon, onClick }: NavItemProps): JSX.Element => (
  <Link
    href={href}
    onClick={onClick}
    className="flex items-center gap-2 py-2 px-4 text-tx-secondary hover:bg-gray-100 hover:text-primary rounded-md transition duration-300"
  >
    {icon} {label}
  </Link>
);
...
const MobileMenu = ({ onClose }: { onClose: () => void }): JSX.Element => (
  <div className="fixed top-24 left-0 right-0 bg-navbar shadow-xs rounded-b-md overflow-hidden z-50 transition-all duration-300">
