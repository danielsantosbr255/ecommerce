// src/services/payment/PayPalPaymentService.js - Concrete implementation for PayPal
...
class PayPalPaymentService extends PaymentService {
  ...
  async createOrder(orderData) {
    const { orderId, totalAmount, currency = "USD", returnUrl, cancelUrl } = orderData;
  ...
  async getPaymentDetails(paymentId) {
    ...
    const response = await axios.get(`${this.baseUrl}/v2/checkout/orders/${paymentId}`, {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    });
