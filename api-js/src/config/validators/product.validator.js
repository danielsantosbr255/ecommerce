...
const name = z.string({ required_error: msg.required }).min(5, msg.minLength(5));
...
module.exports = {
    create(data) {
        return z.object({ name, price, stock, description }).parse(data);
    },

    update(data) {
        let result = z.object({ name, price, stock, description }).partial();
